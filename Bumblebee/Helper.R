...
getDate <- function(dateInt = NA, year = NA, month = NA, day = NA) {
    if (is.wholenumber(dateInt) && dateInt > 19700000) {
...
mergeMainSub <- function(mainTicks, subTicks) {
    ...
    merged <- merge(subTicks, mainTicks, by = 'utcQuoteTime', suffixes = c(".sub", ".main"), all = T)
...
get1LevelTicks <- function(contractId = 'AU1606', exchangeID = 'SHFE', date = 20160310, session = 'D', tickSource = '\\\\172.30.50.120\\nautilusdata\\') {
  ...
  sessions <- sessionMap[productID]
    ...
    a <- a[x,]
        ...
        return(subset(a, select = c('utcQuoteTime', 'mid', 'bb1', 'ba1', 'bbz1', 'baz1')))
    }, error = function(e) {
...
tradeSpread <- function(merged, main, sub, orderSize = 1,hedgeRatio = 1, positionLimit = 10, distortStep = 10, distortPower = 1
    , tradeCoolDown = 30000, timeDiff = 20000, fee = 6, slippage = 1.5,subTrigger = T) {
