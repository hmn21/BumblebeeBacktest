...
createSig <- function(merged, bars, maPeriod, sdAmplifier, maType = 'SMA') {
    #bars <- ticksToBars(merged, samplePeriod = 60)
    x <- data.frame(date = bars$Date, utcQuoteTime = bars$End, BBands(bars$Close, n = maPeriod, maType = maType, sd = sdAmplifier), stringsAsFactors = F)
    y <- merged[, c('date', 'utcQuoteTime')]
    z <- merge(y, x, by = c('date', 'utcQuoteTime'), all = T)
    # bars <- cbind(bars, BBands(bars$Close, n = maPeriod, maType = maType, sd = sdAmplifier))

    merged['dn'] <- repeat.before(z$dn)
    merged['mavg'] <- repeat.before(z$mavg)
    merged['up'] <- repeat.before(z$up)
    merged['pctB'] <- repeat.before(z$pctB)
    ...
    for (i in 1:nrow(merged)) {
        ...
        tryCatch({
            if (is.na(r$mavg)) {
                ...
                if (lastTradeDate == r$date && r$utcQuoteTime - lastTradeTime < tradeCoolDown) {
                next
                }
